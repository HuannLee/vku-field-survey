...
if (btnGps) {
  btnGps.addEventListener("click", async () => {
    try {
      btnGps.querySelector(".btn-inner").textContent = "ĐANG TÌM...";
      
      // Nếu chạy trên app Native qua Capacitor
      if (Geolocation) {
      ...
      alert("Lỗi khi kích hoạt cảm biến GPS!");
    } finally {
      btnGps.querySelector(".btn-inner").textContent = "LẤY GPS";
    }
  });
}

// --- XỬ LÝ CHỤP ẢNH VÀ LẤY BASE64 ---
...
if (btnCamera) {
  btnCamera.addEventListener("click", async () => {
    try {
      if (Camera) {
        // Gọi Camera Native với CameraResultType.Base64 theo mẫu bài học Week 4
        const image = await Camera.getPhoto({
          quality: 70, // Nén nhẹ 70% để gửi mạng nhanh hơn
          allowEditing: false,
          resultType: CameraResultType.Base64,
          source: CameraSource.Camera,
          saveToGallery: true // Lưu vào Album thiết bị
        });
        ...
        btnCamera.querySelector(".btn-inner").textContent = "📷 CHỤP LẠI ẢNH KHÁC";
      } else {
        alert("Camera Plugin chỉ hoạt động trên thiết bị Android/iOS!");
      }
    } catch (error) {
      console.log("Hủy chụp ảnh hoặc lỗi:", error);
    }
  });
}
...
if (btnRemovePhoto) {
  btnRemovePhoto.addEventListener("click", () => {
    ...
    btnCamera.querySelector(".btn-inner").textContent = "📷 CHỤP ẢNH HIỆN TRƯỜNG";
  });
}